...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let array=[];
  arr.forEach (num =>{ array.push(Math.pow(num,3))
...
const addQuestion = (arr) => {
  // Solution code here..
  return arr.map (stringe => stringe +'?')
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here..
  let array = [];
  arr.forEach(num => {array.push(Math.pow(2, num))});
  return array;
};
...
const extractAbilities = (arr) => {
  const abilityName = [];
  arr.map(key => abilityName.push(key.ability.name));
  return abilityName;
};
