...
function transformToLis(obj){
  let arried = Object.entries(obj);
  let toReturn = [];
  for(let i = 0; i < arried.length; i++){
  for(let j = 0; j < arried[i].length-1; j ++){
  toReturn.push(`<li>${arried[i][j]}: ${arried[i][j+1]}</li>`)
   }
  }
  return toReturn;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let finalArrayToReturn = input.map(item => item.filter(element => element%5 === 0))
  finalArrayToReturn = finalArrayToReturn.map(item => item.filter(element => typeof(element) === 'number'))
  return finalArrayToReturn.map(item => item.map(element => Math.pow(2, element)))
};
